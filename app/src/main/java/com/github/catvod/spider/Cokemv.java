...
public class Cokemv extends Spider {
    ...
    public void init(Context context) {
        super.init(context);
        try {
            playerConfig = new JSONObject("{\"ffm3u8\":{\"sh\":\"藍光路线(暫時)\",\"pu\":\"\",\"sn\":0,\"or\":999},\"cokebb\":{\"sh\":\"極速路線\",\"pu\":\"\",\"sn\":0,\"or\":999},\"cokemv\":{\"sh\":\"高速路線\",\"pu\":\"\",\"sn\":0,\"or\":999},\"cokemv001\":{\"sh\":\"海外線路\",\"pu\":\"https://cokemv.me/player/?url=\",\"sn\":0,\"or\":999},\"haiwaikan\":{\"sh\":\"海外線路\",\"pu\":\"\",\"sn\":0,\"or\":999},\"mp4\":{\"sh\":\"藍光路线\",\"pu\":\"\",\"sn\":0,\"or\":999},\"tkm3u8\":{\"sh\":\"采集路线\",\"pu\":\"https://cd.cokemv.me/player/?url=\",\"sn\":0,\"or\":999},\"if101\":{\"sh\":\"海外(禁國內)\",\"pu\":\"https://cd.cokemv.me/player/?url=\",\"sn\":0,\"or\":999},\"777_\":{\"sh\":\"蓝光不卡\",\"pu\":\"十三號線路\",\"sn\":0,\"or\":999}}");
    ...
    public String detailContent(List<String> ids) {
        try {
            ...
            Elements data = doc.select("div.module-info-main div.module-info-heading div.module-info-tag-link");
            category = data.get(2).select("a").text();
            area = data.get(1).select("a").text();
            year = data.get(0).select("a").text();
            remark = doc.select("div.module-info-content div.module-info-items div.module-info-item").get(4).text();
            List<String> actors = new ArrayList<>();
            Elements aa = doc.select("div.module-info-content div.module-info-items div.module-info-item").get(2).select("a");
            for (int j = 0; j < aa.size(); j++) {
                actors.add(aa.get(j).text());
            }
            actor = TextUtils.join(",", actors);
            List<String> directors = new ArrayList<>();
            Elements bb = doc.select("div.module-info-content div.module-info-items div.module-info-item").get(0).select("a");
            for (int j = 0; j < bb.size(); j++) {
                directors.add(bb.get(j).text());
            }
            director = TextUtils.join(",", directors);
            desc = doc.selectFirst("div.module-info-item div.module-info-introduction-content>p").text().trim();
